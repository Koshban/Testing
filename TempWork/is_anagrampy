...
def is_anagrams_using_sorted(strings : list) -> bool:
    print(strings)
    if len(strings) == 2:
        try:
...
def is_anagram_using_hashmaps(t: str, s: str) -> bool:
    t = re.sub(r'[^a-zA-Z]','', t)
    s = re.sub(r'[^a-zA-Z]','', s)
    dict_t, dict_s = {}, {}
    ...
    ({'input': ["restful", "fluster"], 'expected': True})
        ...
        for test_case in test_data:
            with self.subTest(test_case=test_case):
                actual = is_anagram_using_hashmaps(t=test_case["input"][0], s=test_case["input"][1])
